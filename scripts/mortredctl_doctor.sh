...
# `mortredctl doctor`). Wraps verify_deployment.sh --live; falls back to
# --full when the gateway is not reachable so the output still tells the
# operator WHAT is wrong instead of an empty failure.
set -euo pipefail
