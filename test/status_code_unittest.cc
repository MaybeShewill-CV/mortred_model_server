...
#define CHECK_MESSAGE(name, value, desc) \
    EXPECT_EQ(status_code_to_str(StatusCode::name), std::string(desc));
...
TEST(status_code, sample_messages) {
    ...
    EXPECT_EQ(status_code_to_str(StatusCode::TRT_CONVERT_ONNX_MODEL_FAILED),
              "convert onnx model to trt failed");
}
