...
TEST(EnlightenGanPreprocess, ResizePathKeepsRgbChannelOrder) {
    ...
    // solid pure-red BGR image aligned up 300x400 -> 304x400
    ...
    ASSERT_EQ(tensors[0].tensor.shape, (std::vector<int64_t>{1, 3, 304, 400}));
    ASSERT_EQ(tensors[1].tensor.shape, (std::vector<int64_t>{1, 1, 304, 400}));
    ...
    const size_t plane = 304u * 400u;
    for (const auto [row, col] : {std::pair<int, int>{0, 0}, {128, 128}, {303, 399}}) {
        const size_t idx = static_cast<size_t>(row) * 400 + col;
