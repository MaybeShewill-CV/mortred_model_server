...
TEST(cv_utils, calc_iou) {
    ...
    b.score = 1.0f;
    b.class_id = 0;
    EXPECT_NEAR(cv_utils::calc_iou(a, b), 36.0 / 164.0, 1e-5);
    ...
    c.score = 1.0f;
    c.class_id = 0;
    EXPECT_NEAR(cv_utils::calc_iou(a, c), 0.0, 1e-6);
    ...
    zero.score = 1.0f;
    zero.class_id = 0;
    EXPECT_EQ(cv_utils::calc_iou(zero, zero), 0.0f);
    EXPECT_EQ(cv_utils::calc_iou(zero, a), 0.0f);

    // rect overload
    EXPECT_NEAR(cv_utils::calc_iou(cv::Rect2f(0, 0, 10, 10), cv::Rect2f(5, 5, 10, 10)),
                36.0 / 164.0, 1e-5);
}
...
TEST(cv_utils, nms_bboxes) {
    ...
    auto result = cv_utils::nms_bboxes(boxes, 0.5);
...
TEST(cv_utils, convert_chw_hwc_round_trip) {
    ...
    auto chw = cv_utils::convert_to_chw_vec(input);
    ...
    auto hwc = cv_utils::convert_to_hwc_vec(chw, 3, 2, 2);
...
TEST(cv_utils, base64_cvmat_round_trip) {
    ...
    auto encoded = cv_utils::encode_cvmat_into_base64_str(input);
    EXPECT_FALSE(encoded.empty());

    auto decoded = cv_utils::decode_base64_str_into_cvmat(encoded);
...
TEST(cv_utils, colorize_and_stack) {
    ...
    cv::Mat color_mask;
    cv_utils::colorize_segmentation_mask(seg_mask, color_mask, 3);
    ...
    cv::Mat depth_color;
    cv_utils::colorize_depth_map(depth, depth_color);
    ...
    cv::Mat zero_color;
    cv_utils::colorize_depth_map(zero_depth, zero_color);
    EXPECT_EQ(zero_color.size(), zero_depth.size());

    auto color_map = cv_utils::generate_color_map(5);
    ...
    auto stacked = cv_utils::stack_multiple_ddpm_images(images, 0, 2);
...
TEST(cv_utils, generate_color_map_large_class_count) {
    auto color_map = cv_utils::generate_color_map(1024);
...
TEST(cv_utils, colorize_sam_mask_max_label) {
    ...
    cv::Mat color_mask;
    cv_utils::colorize_sam_everything_mask(mask, color_mask);
