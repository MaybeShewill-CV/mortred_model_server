...
private:
    ...
    // worker queue
    moodycamel::ConcurrentQueue<SuperPointPtr> _m_working_queue;
private:
    struct seriex_ctx {
...
StatusCode SuperpointFpServer::Impl::init(const decltype(toml::parse("")) &config) {
    // init working queue
    auto worker_nums = static_cast<int>(config.at("SUPERPOINT_FP_SERVER").at("worker_nums").as_integer());
    auto model_cfg_path = config.at("SUPERPOINT").at("model_config_file_path").as_string();
...
void SuperpointFpServer::Impl::serve_process(WFHttpTask* task) {
    ...
    if (strcmp(task->get_req()->get_request_uri(), "/morted_ai_server_v1/feature_point/superpoint") == 0) {
        ...
        // do classification
        auto&& go_proc = std::bind(&SuperpointFpServer::Impl::do_detection, this, det_task_req, ctx);
        auto* cls_task = WFTaskFactory::create_go_task("superpoint_fp_work", go_proc, det_task_req, ctx);
        *series << cls_task;
    }
}
 ...
 * @return
 */
SuperpointFpServer::Impl::det_request SuperpointFpServer::Impl::parse_task_request(const std::string& req_body) {
 ...
 * @param ctx
 */
void SuperpointFpServer::Impl::do_detection(const det_request& req, seriex_ctx* ctx) {
    // get task receive timestamp
    auto task_receive_ts = Timestamp::now();
    // get resnet model
    SuperPointPtr worker;
...
/****************** SuperpointFpServer Implementation **************/
...
morted::common::StatusCode SuperpointFpServer::init(const decltype(toml::parse("")) &config) {
    ...
    toml::value cfg_content = config.at("SUPERPOINT_FP_SERVER");
    ...
    auto&& proc = std::bind(&SuperpointFpServer::Impl::serve_process, std::cref(this->_m_impl), std::placeholders::_1);
