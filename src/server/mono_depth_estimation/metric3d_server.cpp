 ...
 * @return
 */
StatusCode Metric3DServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["METRIC3D_ESTIMATION_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section METRIC3D_ESTIMATION_SERVER missing or not a table";
 ...
 * @return
 */
jinq::common::StatusCode Metric3DServer::init(const toml::table &config) {
    // init impl
