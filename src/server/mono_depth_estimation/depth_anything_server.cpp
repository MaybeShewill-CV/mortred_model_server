 ...
 * @return
 */
StatusCode DepthAnythingServer::Impl::init(const decltype(toml::parse("")) &config) {
    // init working queue
    auto server_section = config.at("DEPTH_ANYTHING_ESTIMATION_SERVER");
    auto worker_nums = static_cast<int>(server_section.at("worker_nums").as_integer());
    auto model_section = config.at("DEPTH_ANYTHING");
    auto model_cfg_path = model_section.at("model_config_file_path").as_string();

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(FATAL) << "depth anything model config file not exist: " << model_cfg_path;
 ...
 * @return
 */
jinq::common::StatusCode DepthAnythingServer::init(const decltype(toml::parse("")) &config) {
    // init impl
    ...
    auto&& proc = std::bind(
        &DepthAnythingServer::Impl::serve_process, std::cref(this->_m_impl), std::placeholders::_1);
