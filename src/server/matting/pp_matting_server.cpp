 ...
 * @return
 */
StatusCode PPMattingServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["PP_MATTING_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section PP_MATTING_SERVER missing or not a table";
    ...
    auto model_cfg_path = config["PP_MATTING"]["model_config_file_path"].value_or<std::string>("");

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "pp matting model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
 ...
 * @return
 */
void PPMattingServer::Impl::fill_response_data(
    rapidjson::Document::AllocatorType& allocator,
    rapidjson::Document& data,
    const StatusCode& status,
 ...
 * @return
 */
jinq::common::StatusCode PPMattingServer::init(const toml::table &config) {
    // init impl
