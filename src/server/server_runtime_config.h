...
inline StatusCode parse_server_security_config(const toml::table& server_section,
    ...
    cfg.auth_token = server_section["auth_token"].value_or<std::string>("");
    if (const char* env = std::getenv("MORTRED_AUTH_TOKEN"); env != nullptr && *env != '\0') {
        cfg.auth_token = env;
    }
    cfg.rate_limit_qps = static_cast<int>(server_section["rate_limit_qps"].value_or<int64_t>(0));
...
inline StatusCode parse_server_runtime_config(const toml::table& server_section,
    ...
    cfg.max_connections = static_cast<int>(server_section["max_connections"].value_or<int64_t>(0));
    cfg.peer_resp_timeout =
        static_cast<int>(server_section["peer_resp_timeout"].value_or<int64_t>(0)) * 1000;
    cfg.compute_threads = static_cast<int>(server_section["compute_threads"].value_or<int64_t>(0));
    cfg.handler_threads = static_cast<int>(server_section["handler_threads"].value_or<int64_t>(0));
    ...
    cfg.model_run_timeout =
        static_cast<int>(server_section["model_run_timeout"].value_or<int64_t>(500));
    ...
    cfg.stuck_worker_action = server_section["stuck_worker_action"].value_or<std::string>("log");
    ...
    if (cfg.stuck_worker_threshold_times <= 0) {
        cfg.stuck_worker_threshold_times = 3;
    }
    cfg.max_queue_depth = static_cast<int>(server_section["max_queue_depth"].value_or<int64_t>(0));
    if (cfg.max_queue_depth < 0) {
        LOG(WARNING) << "max_queue_depth < 0: queue depth limit disabled";
        cfg.max_queue_depth = 0;
    }
    cfg.max_batch_size = static_cast<int>(server_section["max_batch_size"].value_or<int64_t>(1));
    if (cfg.max_batch_size < 1) {
        LOG(WARNING) << "max_batch_size < 1: batching disabled";
        cfg.max_batch_size = 1;
    }
    cfg.max_batch_delay_ms =
    ...
    if (cfg.max_request_items < 1) {
        ...
        cfg.max_request_items = 16;
    }
    cfg.ewma_seed_ms = cfg.model_run_timeout > 0 ? cfg.model_run_timeout : 500;
    cfg.async_enabled = server_section["async_enabled"].value_or<bool>(false);
    cfg.async_timeout =
