 ...
 * @return
 */
StatusCode BiseNetV2Server::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["BISENETV2_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section BISENETV2_SERVER missing or not a table";
    ...
    auto model_cfg_path = config["BISENETV2"]["model_config_file_path"].value_or<std::string>("");

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "bisenetv2 model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
 ...
 * @return
 */
void BiseNetV2Server::Impl::fill_response_data(
    rapidjson::Document::AllocatorType& allocator,
    rapidjson::Document& data,
    const StatusCode& status,
 ...
 * @return
 */
jinq::common::StatusCode BiseNetV2Server::init(const toml::table &config) {
    // init impl
