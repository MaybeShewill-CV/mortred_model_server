 ...
 * @return
 */
StatusCode RealEsrGanServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["REAL_ESRGAN_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section REAL_ESRGAN_SERVER missing or not a table";
    ...
    const toml::table* model_section_ptr = config["REAL_ESRGAN"].as_table();
    if (model_section_ptr == nullptr) {
        LOG(ERROR) << "Config section REAL_ESRGAN missing or not a table";
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
    ...
    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "real esr-gan model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
 ...
 * @return
 */
void RealEsrGanServer::Impl::fill_response_data(
    rapidjson::Document::AllocatorType& allocator,
    rapidjson::Document& data,
    const StatusCode& status,
 ...
 * @return
 */
jinq::common::StatusCode RealEsrGanServer::init(const toml::table &config) {
    // init impl
