 ...
 * @return
 */
StatusCode AttentiveGanDerainServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["ATTENTIVE_GAN_DERAIN_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section ATTENTIVE_GAN_DERAIN_SERVER missing or not a table";
    ...
    auto model_cfg_path = config["ATTENTIVE_GAN_DERAIN"]["model_config_file_path"].value_or<std::string>("");

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "attentive gan model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
 ...
 * @return
 */
void AttentiveGanDerainServer::Impl::fill_response_data(
    rapidjson::Document::AllocatorType& allocator,
    rapidjson::Document& data,
    const StatusCode& status,
    ...
    if (model_output.enhancement_result.empty()) {
        data.AddMember("enhance_result", "", allocator);
        return;
    }
 ...
 * @return
 */
jinq::common::StatusCode AttentiveGanDerainServer::init(const toml::table &config) {
    // init impl
