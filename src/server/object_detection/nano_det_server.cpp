...
class NanoDetServer::Impl : public BaseAiServerImpl<NanoDetPtr, std_object_detection_output> {
public:
    /***
    *
    * @param cfg_file_path
 ...
 * @return
 */
StatusCode NanoDetServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["NANODET_DETECTION_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section NANODET_DETECTION_SERVER missing or not a table";
    ...
    auto model_cfg_path = config["NANODET"]["model_config_file_path"].value_or<std::string>("");

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "nanodet model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
 ...
 * @return
 */
void NanoDetServer::Impl::fill_response_data(
    rapidjson::Document::AllocatorType& allocator,
    rapidjson::Document& data,
    const StatusCode& status,
 ...
 * @return
 */
jinq::common::StatusCode NanoDetServer::init(const toml::table &config) {
    // init impl
    auto status = _m_impl->init(config);
    if (status != StatusCode::OK) {
        LOG(INFO) << "init nanodet detection server failed";
        return status;
    }
