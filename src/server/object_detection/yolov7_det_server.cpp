 ...
 * @return
 */
StatusCode YoloV7DetServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["YOLOV7_DETECTION_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section YOLOV7_DETECTION_SERVER missing or not a table";
    ...
    auto model_cfg_path = config["YOLOV7"]["model_config_file_path"].value_or<std::string>("");

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "yolov7 model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
 ...
 * @return
 */
void YoloV7DetServer::Impl::fill_response_data(
    rapidjson::Document::AllocatorType& allocator,
    rapidjson::Document& data,
    const StatusCode& status,
 ...
 * @return
 */
jinq::common::StatusCode YoloV7DetServer::init(const toml::table &config) {
    // init impl
