...
private:
    ...
    // worker queue
    moodycamel::ConcurrentQueue<Yolov5DetPtr> _m_working_queue;
private:
    struct seriex_ctx {
...
StatusCode YoloV5DetServer::Impl::init(const decltype(toml::parse("")) &config) {
    // init working queue
    auto worker_nums = static_cast<int>(config.at("YOLOV5_DETECTION_SERVER").at("worker_nums").as_integer());
 ...
 * @param task
 */
void YoloV5DetServer::Impl::serve_process(WFHttpTask* task) {
    // welcome message
    if (strcmp(task->get_req()->get_request_uri(), "/welcome") == 0) {
        task->get_resp()->append_output_body("<html>Welcome to mortred Yolov5 Object Detection Server</html>");
        return;
    }
    ...
    if (strcmp(task->get_req()->get_request_uri(), "/mortred_ai_server_v1/obj_detection/yolov5") == 0) {
        // parse request body
        ...
        // do classification
        auto&& go_proc = std::bind(&YoloV5DetServer::Impl::do_detection, this, det_task_req, ctx);
        auto* cls_task = WFTaskFactory::create_go_task("yolov5_det_work", go_proc, det_task_req, ctx);
        *series << cls_task;
    }
}
 ...
 * @return
 */
YoloV5DetServer::Impl::det_request YoloV5DetServer::Impl::parse_task_request(const std::string& req_body) {
 ...
 * @param ctx
 */
void YoloV5DetServer::Impl::do_detection(const det_request& req, seriex_ctx* ctx) {
...
/****************** YoloV5DetServer Implementation **************/
...
mortred::common::StatusCode YoloV5DetServer::init(const decltype(toml::parse("")) &config) {
    ...
    toml::value cfg_content = config.at("YOLOV5_DETECTION_SERVER");
    ...
    auto&& proc = std::bind(&YoloV5DetServer::Impl::serve_process, std::cref(this->_m_impl), std::placeholders::_1);
