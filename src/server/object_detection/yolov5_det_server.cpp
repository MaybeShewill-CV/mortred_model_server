...
void YoloV5DetServer::Impl::serve_process(WFHttpTask* task) {
    ...
    if (strcmp(task->get_req()->get_request_uri(), "/welcome") == 0) {
        task->get_resp()->append_output_body("<html>Welcome to Morted Yolov5 Object Detection Server</html>");
        return;
    }
    ...
    if (strcmp(task->get_req()->get_request_uri(), "/morted_ai_server_v1/obj_detection/yolov5") == 0) {
        // parse request body
