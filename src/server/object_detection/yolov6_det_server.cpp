...
* File: yolov6_det_server.cpp
* Date: 23-3-3
************************************************/

#include "yolov6_det_server.h"
...
using jinq::factory::object_detection::create_yolov6_detector;
using jinq::models::io_define::common_io::base64_input;
using jinq::models::io_define::object_detection::std_object_detection_output;
using Yolov6DetPtr = decltype(create_yolov6_detector<base64_input, std_object_detection_output>(""));

class YoloV6DetServer::Impl : public BaseAiServerImpl<Yolov6DetPtr, std_object_detection_output> {
public:
    /***
    *
    * @param cfg_file_path
 ...
 * @return
 */
StatusCode YoloV6DetServer::Impl::init(const toml::table &config) {
    // init working queue
    const toml::table* server_section_ptr = config["YOLOV6_DETECTION_SERVER"].as_table();
    if (server_section_ptr == nullptr) {
        LOG(ERROR) << "Config section YOLOV6_DETECTION_SERVER missing or not a table";
    ...
    auto model_cfg_path = config["YOLOV6"]["model_config_file_path"].value_or<std::string>("");

    if (!FilePathUtil::is_file_exist(model_cfg_path)) {
        LOG(ERROR) << "yolov6 model config file not exist: " << model_cfg_path;
        _m_successfully_initialized = false;
        return StatusCode::SERVER_INIT_FAILED;
    }
    ...
    for (int index = 0; index < worker_nums; ++index) {
        auto worker = create_yolov6_detector<base64_input, std_object_detection_output>(
    ...
    LOG(INFO) << "Yolov6 object detection server init successfully";
 ...
 *
 */
YoloV6DetServer::YoloV6DetServer() {
 ...
 *
 */
YoloV6DetServer::~YoloV6DetServer() = default;
 ...
 * @return
 */
jinq::common::StatusCode YoloV6DetServer::init(const toml::table &config) {
    // init impl
    auto status = _m_impl->init(config);
    if (status != StatusCode::OK) {
        LOG(INFO) << "init yolov6 detection server failed";
        return status;
    }
 ...
 * @param task
 */
void YoloV6DetServer::serve_process(WFHttpTask* task) {
 ...
 * @return
 */
bool YoloV6DetServer::is_successfully_initialized() const {
