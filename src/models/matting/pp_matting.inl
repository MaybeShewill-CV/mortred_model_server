...
StatusCode PPMatting<INPUT, OUTPUT>::on_init(const toml::table& params) {
    (void)params;
    const auto& input_info = this->session().inputs().front();
    if (input_info.shape.size() != 4 || input_info.shape[1] != 3) {
        LOG(ERROR) << "unexpected ppmatting input shape: " << input_info.to_string()
...
template<typename INPUT, typename OUTPUT>
std::vector<NamedTensor> PPMatting<INPUT, OUTPUT>::preprocess(const cv::Mat& input_image) {
...
template<typename INPUT, typename OUTPUT>
StatusCode PPMatting<INPUT, OUTPUT>::postprocess(
    ...
    if (tensor.element_count() < static_cast<int64_t>(_m_input_size_host.area())) {
        LOG(ERROR) << "unexpected ppmatting output shape: "
...
PPMatting<INPUT, OUTPUT>::PPMatting()
    : jinq::models::BackendCvModel<INPUT, OUTPUT>("PP_MATTING") {}
