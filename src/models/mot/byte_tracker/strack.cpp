 ...
 * @param f_id
 */
void STrack::activate(KalmanFilter &kalman_filter, int f_id) {
    _m_kalman_filter = kalman_filter;
 ...
 * @param new_id
 */
void STrack::re_activate(STrack &new_track, int f_id, bool new_id) {
    ...
    xyah_box[2] = xyah[2];
    xyah_box[3] = xyah[3];
    auto mc = _m_kalman_filter.update(this->mean, this->covariance, xyah_box);
    this->mean = mc.first;
    this->covariance = mc.second;
 ...
 * @param f_id
 */
void STrack::update(STrack &new_track, int f_id) {
    this->frame_id = f_id;
    this->tracklet_len++;
