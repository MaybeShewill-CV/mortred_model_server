...
{
    common_out result;
    result.class_id = internal_output.class_id;
    result.scores = internal_output.scores;
    return result;
}
...
private:
    ...
    // 模型score阈值
    float _m_score_threshold = 0.9f;
    // 模型是否成功初始化标志位
    bool _m_is_net_successfully_initialized = false;
    // 图像均值
...
StatusCode MobileNetv2<INPUT, OUTPUT>::Impl::init(const decltype(toml::parse(""))& config)
{
    ...
    {
        LOG(ERROR)
                << "Config文件没有MOBILENETV2相关配置, 请重新检查配置文件";
    ...
    {
        LOG(ERROR)
                << "Config doesn\'t have model_file_path field";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }
    else
    ...
    _m_net = std::unique_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile(model_file_path.c_str()));
    if (_m_net == nullptr)
    {
        LOG(ERROR) << "Create Interpreter failed, model file path: " << model_file_path;
        _m_is_net_successfully_initialized = false;
        return;
    }

    if (config.find("model_threads_nums") == config.end())
    {
    ...
    {
        LOG(ERROR) << "Create Session failed, model file path: " << model_file_path;
        _m_is_net_successfully_initialized = false;
        return;
    }
