...
public:
    ...
    */
    ...
    StatusCode run(const INPUT& in, std::vector<OUTPUT>& out) {
        ...
        out.push_back(densenet_impl::transform_output<OUTPUT>(internal_out));
...
StatusCode DenseNet<INPUT, OUTPUT>::Impl::init(const decltype(toml::parse(""))& config) {
    if (!config.contains("DENSENET")) {
        LOG(ERROR) << "Config文件没有DENSENET相关配置, 请重新检查配置文件";
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode DenseNet<INPUT, OUTPUT>::init(const decltype(toml::parse(
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode DenseNet<INPUT, OUTPUT>::run(const INPUT& input, std::vector <OUTPUT>& output) {
