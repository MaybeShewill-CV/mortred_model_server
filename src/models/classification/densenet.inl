...
*/
template<typename INPUT, typename OUTPUT>
StatusCode DenseNet<INPUT, OUTPUT>::Impl::init(const toml::table& config) {
    ...
    const toml::table* cfg_content_ptr = config["DENSENET"].as_table();
    if (cfg_content_ptr == nullptr) {
        LOG(ERROR) << "Config section DENSENET missing or not a table";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode DenseNet<INPUT, OUTPUT>::init(const toml::table& cfg) {
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode DenseNet<INPUT, OUTPUT>::run_impl(const INPUT& input, OUTPUT& output) {
