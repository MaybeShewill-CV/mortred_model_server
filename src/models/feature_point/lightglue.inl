...
using FeatureMatchOutput =
    jinq::models::io_define::feature_point::std_feature_point_match_output;
...
StatusCode LightGlue<INPUT, OUTPUT>::on_init(const toml::table& params) {
    if (params.contains("extract_score_thresh")) {
        _m_extract_score_threshold =
            static_cast<float>(params["extract_score_thresh"].value_or<double>(0.0));
    }
    if (params.contains("match_score_thresh")) {
        _m_match_score_threshold =
            static_cast<float>(params["match_score_thresh"].value_or<double>(0.0));
    }
    if (params.contains("long_side_length")) {
        _m_long_side_length =
            static_cast<float>(params["long_side_length"].value_or<double>(0.0));
    }
...
template<typename INPUT, typename OUTPUT>
const TensorInfo* LightGlue<INPUT, OUTPUT>::find_info(
    ...
    const auto input_iter = std::find_if(
        session.inputs().begin(), session.inputs().end(), search_input);
    ...
    const auto output_iter = std::find_if(
        session.outputs().begin(), session.outputs().end(), search_output);
...
template<typename INPUT, typename OUTPUT>
StatusCode LightGlue<INPUT, OUTPUT>::validate_io(const InferenceSession& session) {
    struct ExpectedIo {
        const char* name;
        bool is_input;
    ...
    const std::vector<ExpectedIo> expected = is_extractor
        ? std::vector<ExpectedIo>{
              {"image", true, jinq::models::backend::DType::F32, 4},
        ...
        : std::vector<ExpectedIo>{
              {"kpts0", true, jinq::models::backend::DType::F32, 3},
              {"kpts1", true, jinq::models::backend::DType::F32, 3},
              {"desc0", true, jinq::models::backend::DType::F32, 3},
              {"desc1", true, jinq::models::backend::DType::F32, 3},
              {"matches0", false, jinq::models::backend::DType::I32, 2},
              {"mscores0", false, jinq::models::backend::DType::F32, 1}};
    ...
    for (const auto& item : expected) {
        ...
        const bool correct_side =
            item.is_input
                ? std::any_of(session.inputs().begin(), session.inputs().end(),
...
template<typename INPUT, typename OUTPUT>
const NamedTensor* LightGlue<INPUT, OUTPUT>::find_output(
...
template<typename INPUT, typename OUTPUT>
cv::Mat LightGlue<INPUT, OUTPUT>::preprocess_image(const cv::Mat& input_image) const {
    ...
    const auto resize_height =
        static_cast<int>(static_cast<float>(input_image.rows) * resize_scale);
    const auto resize_width =
        static_cast<int>(static_cast<float>(input_image.cols) * resize_scale);
    if (resize_height <= 0 || resize_width <= 0) {
        LOG(ERROR) << "lightglue resized image size is empty: " << resize_width << "x"
                   << resize_height;
        return {};
    }
...
template<typename INPUT, typename OUTPUT>
StatusCode LightGlue<INPUT, OUTPUT>::extract_feature_points(
    ...
    NamedTensor image;
    image.name = "image";
    image.tensor = Tensor::make<float>(
        {1, 1, input_image.rows, input_image.cols});
    ...
    if (keypoints->tensor.dtype != jinq::models::backend::DType::I32 ||
        scores->tensor.dtype != jinq::models::backend::DType::F32 ||
    ...
    if (score_count == 0 ||
        keypoints->tensor.element_count() != static_cast<int64_t>(score_count * 2) ||
        descriptors->tensor.element_count() != static_cast<int64_t>(score_count * 256)) {
        LOG(ERROR) << "lightglue extractor output sizes mismatch: scores "
                   << score_count << ", keypoints " << keypoints->tensor.element_count()
                   << ", descriptors " << descriptors->tensor.element_count();
        return StatusCode::MODEL_EMPTY_OUTPUT;
    }
    ...
    for (size_t idx = 0; idx < score_count; ++idx) {
        ...
        feature_points.normalized_keypoints.push_back(
            (static_cast<float>(keypoint_data[idx * 2]) - input_image.cols * 0.5f) /
            static_cast<float>(normalize_scale));
        feature_points.normalized_keypoints.push_back(
            (static_cast<float>(keypoint_data[idx * 2 + 1]) - input_image.rows * 0.5f) /
            static_cast<float>(normalize_scale));
        feature_points.descriptors.insert(
            feature_points.descriptors.end(), descriptor_data + idx * 256,
            descriptor_data + (idx + 1) * 256);
    }
    return StatusCode::OK;
}

template<typename INPUT, typename OUTPUT>
StatusCode LightGlue<INPUT, OUTPUT>::match_feature_points(
    const FeaturePoints& src_features, const FeaturePoints& dst_features,
    ...
    if (src_features.descriptors.size() != src_count * 256 ||
        dst_features.descriptors.size() != dst_count * 256 ||
    ...
    std::memcpy(kpts0.tensor.buffer.data(), src_features.normalized_keypoints.data(),
                kpts0.tensor.byte_size());
    inputs.push_back(std::move(kpts0));

    NamedTensor kpts1;
    kpts1.name = "kpts1";
    kpts1.tensor = Tensor::make<float>({1, static_cast<int64_t>(dst_count), 2});
    std::memcpy(kpts1.tensor.buffer.data(), dst_features.normalized_keypoints.data(),
                kpts1.tensor.byte_size());
    inputs.push_back(std::move(kpts1));

    NamedTensor desc0;
    desc0.name = "desc0";
    desc0.tensor =
        Tensor::make<float>({1, static_cast<int64_t>(src_count), 256});
    std::memcpy(desc0.tensor.buffer.data(), src_features.descriptors.data(),
                desc0.tensor.byte_size());
    inputs.push_back(std::move(desc0));

    NamedTensor desc1;
    desc1.name = "desc1";
    desc1.tensor =
        Tensor::make<float>({1, static_cast<int64_t>(dst_count), 256});
    std::memcpy(desc1.tensor.buffer.data(), dst_features.descriptors.data(),
                desc1.tensor.byte_size());
    ...
    if (match_indices->tensor.element_count() != static_cast<int64_t>(score_count * 2)) {
        LOG(ERROR) << "lightglue matcher output sizes mismatch: matches "
                   << match_indices->tensor.element_count() << ", scores " << score_count;
        return StatusCode::MODEL_EMPTY_OUTPUT;
    }
    ...
    for (size_t idx = 0; idx < score_count; ++idx) {
        ...
        if (src_index < 0 || static_cast<size_t>(src_index) >= src_count ||
            dst_index < 0 || static_cast<size_t>(dst_index) >= dst_count) {
            continue;
        }

        namespace feature_point_io = jinq::models::io_define::feature_point;
        feature_point_io::fp src_point;
        src_point.location = cv::Point2f(
            static_cast<float>(src_features.keypoints[static_cast<size_t>(src_index) * 2]),
            static_cast<float>(
                src_features.keypoints[static_cast<size_t>(src_index) * 2 + 1]));
        feature_point_io::fp dst_point;
        dst_point.location = cv::Point2f(
            static_cast<float>(dst_features.keypoints[static_cast<size_t>(dst_index) * 2]),
            static_cast<float>(
                dst_features.keypoints[static_cast<size_t>(dst_index) * 2 + 1]));
...
template<typename INPUT, typename OUTPUT>
StatusCode LightGlue<INPUT, OUTPUT>::run_sessions(const INPUT& input, OUTPUT& output) {
    if constexpr (std::is_same_v<INPUT, jinq::models::io_define::common_io::pair_mat_input>) {
        ...
        const auto src_width_scale =
            static_cast<float>(src_size_user.width) / static_cast<float>(src_preprocessed.cols);
        const auto src_height_scale =
            static_cast<float>(src_size_user.height) / static_cast<float>(src_preprocessed.rows);
        const auto dst_width_scale =
            static_cast<float>(dst_size_user.width) / static_cast<float>(dst_preprocessed.cols);
        const auto dst_height_scale =
            static_cast<float>(dst_size_user.height) / static_cast<float>(dst_preprocessed.rows);
...
template<typename INPUT, typename OUTPUT>
StatusCode LightGlue<INPUT, OUTPUT>::postprocess(
...
template<typename INPUT, typename OUTPUT>
LightGlue<INPUT, OUTPUT>::LightGlue()
    : jinq::models::BackendCvModel<INPUT, OUTPUT>("LIGHTGLUE") {}
