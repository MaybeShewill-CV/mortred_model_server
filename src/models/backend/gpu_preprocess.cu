 ...
 * All descriptor fields are passed as scalar kernel params (GPU-friendly).
 * Geometry params (unpad_w/h, pad_x/y, crop offsets) are pre-computed on
 * the host and passed as constants. */
...
__global__ void preprocess_ycbcr_kernel(
    ...
    int cb_w, int cb_h,
    // output
    IS_FP16 ? __half* : float* output,   // resolved by template
    void* gray_output,                     // optional secondary grayscale
    int out_w, int out_h,
    ...
    int unpad_w, int unpad_h,   // effective resized area within the canvas
    int pad_x, int pad_y,       // offset of the image within the canvas
    int crop_x, int crop_y      // CENTER_CROP offsets
)
{
    ...
    case RESIZE_CENTER_CROP: {
        // pre_crop has already been applied conceptually: the crop region
        // is at (crop_x, crop_y) within the pre-cropped image at (pre_crop_w, pre_crop_h)
        // Here we assume the host has computed the full mapping:
        // source → pre_crop_size → crop → network_size
    ...
    case RESIZE_KEEP_RATIO_PAD_CENTER: {
        ...
        if (lx < 0 || lx >= unpad_w || ly < 0 || ly >= unpad_h) {
            is_pad = true;
            pad_value_f = 0.0f;  // both variants pad with zero
        } else {
            sx = (lx * src_w) / unpad_w;
            sy = (ly * src_h) / unpad_h;
        }
    ...
    case RESIZE_NONE: {
        // output size is already the aligned/source size
        sx = (w * src_w) / out_w;
        sy = (h * src_h) / out_h;
        break;
    }
    ...
    int rx = sx, ry = sy;
    switch (rotation) {
    case ROT_90:   rx = sy;           ry = src_w - 1 - sx; break;
    case ROT_180:  rx = src_w - 1 - sx; ry = src_h - 1 - sy; break;
    case ROT_270:  rx = src_h - 1 - sy; ry = sx;            break;
    default: break;
    }
    sx = rx; sy = ry;
    ...
    if (is_pad || sx < 0 || sx >= src_w || sy < 0 || sy >= src_h) {
        const float pv = (pad_val / 255.0f - mean0) / std0;  // approximate
        write_pixel<IS_FP16, IS_NHWC>(output, w, h, out_w, out_h,
                                       pv, pv, pv, color_order);
        return;
    }
    ...
    // ── Step 5: write to output (layout and dtype by template) ──
    write_pixel<IS_FP16, IS_NHWC>(output, w, h, out_w, out_h,
                                   r, g, b, color_order);
    ...
    if (gray_output != nullptr) {
        ...
        if constexpr (IS_FP16) {
            __half* gray_ptr = (__half*)gray_output;
            gray_ptr[(size_t)h * out_w + w] = __float2half(gray);
        } else {
            float* gray_ptr = (float*)gray_output;
            gray_ptr[(size_t)h * out_w + w] = gray;
        }
    }
}

/*** Helper: write one pixel to NCHW or NHWC layout, fp16 or fp32. */
template <bool IS_FP16, bool IS_NHWC>
__device__ __forceinline__ void write_pixel(
    void* output, int w, int h, int out_w, int out_h,
    float r, float g, float b, int color_order)
{
    float ch0 = (color_order == COLOR_BGR) ? b : (color_order == COLOR_GRAY) ? (0.299f*r + 0.587f*g + 0.114f*b) : r;
    float ch1 = (color_order == COLOR_GRAY) ? (0.299f*r + 0.587f*g + 0.114f*b) : g;
    float ch2 = (color_order == COLOR_BGR) ? r : (color_order == COLOR_GRAY) ? (0.299f*r + 0.587f*g + 0.114f*b) : b;
    ...
    cudaStream_t stream)
{
    ...
    // Dispatch table: 4 template combinations
    if (output_is_fp16 && !output_is_nhwc) {
        preprocess_ycbcr_kernel<true, false><<<grid, block, 0, stream>>>(
            ...
            (__half*)d_out, d_gray_out, out_w, out_h,
        ...
        preprocess_ycbcr_kernel<false, false><<<grid, block, 0, stream>>>(
            d_y, d_cb, d_cr, src_w, src_h, cb_w, cb_h,
            (float*)d_out, d_gray_out, out_w, out_h,
            resize_type, color_order, rotation,
            norm_scale, mean0, mean1, mean2, std0, std1, std2,
            pad_val, unpad_w, unpad_h, pad_x, pad_y, crop_x, crop_y);
    } else {
