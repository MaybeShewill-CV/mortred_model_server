...
const TensorInfo *OpenAiClip<INPUT, OUTPUT>::find_info(const InferenceSession &session, const std::string &name) {
    const auto input_iter =
        std::find_if(session.inputs().begin(), session.inputs().end(), [&name](const TensorInfo &info) { return info.name == name; });
    if (input_iter != session.inputs().end()) {
        return &*input_iter;
    }
    const auto output_iter =
        std::find_if(session.outputs().begin(), session.outputs().end(), [&name](const TensorInfo &info) { return info.name == name; });
...
template <typename INPUT, typename OUTPUT> StatusCode OpenAiClip<INPUT, OUTPUT>::validate_visual_io(const InferenceSession &session) {
    ...
    if (input == nullptr || output == nullptr || input->dtype != jinq::models::backend::DType::F32 ||
        output->dtype != jinq::models::backend::DType::F32 || input->shape.size() != 4 || input->shape[1] != 3 ||
...
template <typename INPUT, typename OUTPUT> StatusCode OpenAiClip<INPUT, OUTPUT>::validate_text_io(const InferenceSession &session) {
    const auto *input = find_info(session, "input");
    const auto *output = find_info(session, "output");
    if (input == nullptr || output == nullptr || input->dtype != jinq::models::backend::DType::I32 ||
        output->dtype != jinq::models::backend::DType::F32 || input->shape.size() != 2 || output->shape.size() != 2 || input->dynamic ||
        output->dynamic) {
...
template <typename INPUT, typename OUTPUT>
const NamedTensor *OpenAiClip<INPUT, OUTPUT>::find_output(const std::vector<NamedTensor> &outputs, const std::string &name) const {
...
template <typename INPUT, typename OUTPUT> bool OpenAiClip<INPUT, OUTPUT>::normalize_embedding(std::vector<float> &embedding) {
    const auto squared_norm = std::inner_product(embedding.begin(), embedding.end(), embedding.begin(), 0.0f);
    const auto norm = std::sqrt(squared_norm);
    if (!std::isfinite(norm) || norm <= k_embedding_norm_eps) {
        return false;
    }
    std::transform(embedding.begin(), embedding.end(), embedding.begin(), [norm](float value) { return value / norm; });
    return true;
}

template <typename INPUT, typename OUTPUT>
bool OpenAiClip<INPUT, OUTPUT>::embeddings_compatible(const std::vector<float> &lhs, const std::vector<float> &rhs) {
    return !lhs.empty() && lhs.size() == rhs.size();
}

template <typename INPUT, typename OUTPUT> cv::Mat OpenAiClip<INPUT, OUTPUT>::preprocess_image(const cv::Mat &input_image) const {
    ...
    cv::resize(result, result, cv::Size(static_cast<int>(input_info.shape[3]), static_cast<int>(input_info.shape[2])));
...
template <typename INPUT, typename OUTPUT>
void OpenAiClip<INPUT, OUTPUT>::tokenize(const std::string &input_text, std::vector<int32_t> &token_ids) const {
    token_ids.clear();
...
template <typename INPUT, typename OUTPUT>
StatusCode OpenAiClip<INPUT, OUTPUT>::encode_text(const std::string &input_text, std::vector<float> &text_embeddings) const {
...
template <typename INPUT, typename OUTPUT>
StatusCode OpenAiClip<INPUT, OUTPUT>::texts2img(const std::vector<std::string> &input_texts, const cv::Mat &input_image,
    ...
    for (const auto &text : input_texts) {
        ...
        if (!normalize_embedding(text_embedding) || !embeddings_compatible(text_embedding, visual_embedding)) {
            LOG(ERROR) << "openai clip text/visual embedding shapes are incompatible";
            return StatusCode::MODEL_EMPTY_OUTPUT;
        }
        const auto cosine = std::inner_product(text_embedding.begin(), text_embedding.end(), visual_embedding.begin(), 0.0f);
        logits.push_back(std::min(100.0f * cosine, k_clip_logit_cap));
    }

    const auto max_logit = *std::max_element(logits.begin(), logits.end());
    const auto score_sum = std::accumulate(logits.begin(), logits.end(), 0.0f,
                                           [max_logit](float sum, float value) { return sum + std::exp(value - max_logit); });
...
template <typename INPUT, typename OUTPUT>
StatusCode OpenAiClip<INPUT, OUTPUT>::imgs2text(const std::vector<cv::Mat> &input_images, const std::string &input_text,
                                                std::vector<float> &simi_scores) {
    simi_scores.clear();
    ...
    for (const auto &image : input_images) {
        ...
        if (!normalize_embedding(visual_embedding) || !embeddings_compatible(visual_embedding, text_embedding)) {
            LOG(ERROR) << "openai clip text/visual embedding shapes are incompatible";
            return StatusCode::MODEL_EMPTY_OUTPUT;
        }
        const auto cosine = std::inner_product(text_embedding.begin(), text_embedding.end(), visual_embedding.begin(), 0.0f);
...
template <typename INPUT, typename OUTPUT> StatusCode OpenAiClip<INPUT, OUTPUT>::run_sessions(const INPUT &input, OUTPUT &output) {
    ...
    case ClipTaskType::TEXTS_TO_IMAGE:
        status = texts2img(internal_input.texts, internal_input.image, internal_output.simi_scores);
        break;
    case ClipTaskType::IMAGES_TO_TEXT:
        status = imgs2text(internal_input.images, internal_input.text, internal_output.simi_scores);
        break;
    default:
...
template <typename INPUT, typename OUTPUT>
StatusCode OpenAiClip<INPUT, OUTPUT>::postprocess(const std::vector<jinq::models::backend::NamedTensor> &outputs,
