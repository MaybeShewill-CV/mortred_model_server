...
#include "cls_cond_ddim_sampler.h"
...
using ClsCondDenoiseModelPtr = jinq::models::diffusion::ClsCondDDPMUNet<std_cls_cond_ddpm_unet_input, std_cls_cond_ddpm_unet_output>;

namespace cls_cond_ddim_sampler_impl {

using internal_input = std_cls_cond_ddim_input;
using internal_output = std_cls_cond_ddim_output;
...
typename std::enable_if<std::is_same<INPUT, std::decay<std_cls_cond_ddim_input>::type>::value, internal_input>::type
...
typename std::enable_if<std::is_same<OUTPUT, std::decay<std_cls_cond_ddim_output >::type>::value, std_cls_cond_ddim_output>::type
transform_output(const cls_cond_ddim_sampler_impl::internal_output& internal_out) {
    return internal_out;
}
...
class ClsCondDDIMSampler<INPUT, OUTPUT>::Impl {
  public:
    /***
     *
     */
  ...
  private:
    ...
    // denoise schedule
    std::unique_ptr<ClsCondDenoiseModelPtr> _m_denoise_net;
  ...
  private:
    ...
    std::vector<double> linear_beta_schedule(const int timesteps, const double beta_start=0.0001, const double beta_end=0.02) {
        ...
        return cls_cond_ddim_sampler_impl::linspace(real_beta_start, real_beta_end, static_cast<int>(timesteps));
    }
    ...
    std::vector<double> cos_beta_schedule(const int timesteps, const double s=0.008) {
        ...
        auto t = cls_cond_ddim_sampler_impl::linspace(0, static_cast<double>(timesteps), static_cast<int>(steps));
        for (auto& val : t) {
    ...
    std::vector<std::tuple<std::vector<float>, std::vector<float> > > p_sample(
        std::vector<float>& xt, int total_steps, int sample_steps, int cls_id, float eta=0.0f, bool save_all_mid_results=false);
    ...
    std::tuple<std::vector<float>, std::vector<float> > p_sample_once(
        std::vector<float>& xt, int t, int t_next, int cls_id, float eta=0.0f, bool is_last_step=false);
};
 ...
 */
template <typename INPUT, typename OUTPUT>
StatusCode ClsCondDDIMSampler<INPUT, OUTPUT>::Impl::init(const decltype(toml::parse("")) &config) {
    ...
    // init denoise model
    _m_denoise_net = std::make_unique<ClsCondDenoiseModelPtr>();
 ...
 */
template <typename INPUT, typename OUTPUT>
StatusCode ClsCondDDIMSampler<INPUT, OUTPUT>::Impl::run(const INPUT& in, OUTPUT& out) {
    // prepare input params
    auto transformed_input = cls_cond_ddim_sampler_impl::transform_input(in);
    ...
    std::vector<float> xt;
    if (xt_data == nullptr) {
        xt = cls_cond_ddim_sampler_impl::generate_random_norm_vector(sample_size.area() * sample_channels, 0.0, 1.0);
    } else {
    ...
    // transform output
    out = cls_cond_ddim_sampler_impl::transform_output<OUTPUT>(internal_out);
 ...
 */
template <typename INPUT, typename OUTPUT>
std::vector<std::tuple<std::vector<float>, std::vector<float> > > ClsCondDDIMSampler<INPUT, OUTPUT>::Impl::p_sample(
    std::vector<float>& xt, int total_steps, int sample_steps, int cls_id, float eta, bool save_all_mid_results) {
    // loop sample
    std::vector<std::tuple<std::vector<float>, std::vector<float> > > mid_sample_results;
    std::vector<double> steps = cls_cond_ddim_sampler_impl::linspace(0.0, static_cast<double>(total_steps) - 1.0, sample_steps);
    std::reverse(steps.begin(), steps.end());
    for (auto idx = 0; idx < steps.size(); ++idx) {
        auto t = steps[idx];
        bool is_last = t == 0;
        auto t_next = is_last ? t : steps[idx + 1];
        auto sample_result = p_sample_once(xt, t, t_next, cls_id, eta, is_last);
 ...
 */
template <typename INPUT, typename OUTPUT>
std::tuple<std::vector<float>, std::vector<float> > ClsCondDDIMSampler<INPUT, OUTPUT>::Impl::p_sample_once(
    std::vector<float>& xt, int t, int t_next, int cls_id, float eta, bool is_last_step) {
    // compute predict noise
    std_cls_cond_ddpm_unet_input denoise_in;
    denoise_in.xt = xt;
    denoise_in.timestep = t;
    denoise_in.cls_id = cls_id;
    std_cls_cond_ddpm_unet_output denoise_out;
    ...
    auto random_noise = cls_cond_ddim_sampler_impl::generate_random_norm_vector(xt.size());
 ...
 */
template <typename INPUT, typename OUTPUT>
ClsCondDDIMSampler<INPUT, OUTPUT>::ClsCondDDIMSampler() {
 ...
 */
template <typename INPUT, typename OUTPUT>
ClsCondDDIMSampler<INPUT, OUTPUT>::~ClsCondDDIMSampler() = default;
 ...
 */
template <typename INPUT, typename OUTPUT>
StatusCode ClsCondDDIMSampler<INPUT, OUTPUT>::init(const decltype(toml::parse("")) &cfg) {
 ...
 */
template <typename INPUT, typename OUTPUT>
bool ClsCondDDIMSampler<INPUT, OUTPUT>::is_successfully_initialized() const {
