...
*/
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::Impl::init(const decltype(toml::parse(""))& config) {
    ...
    toml::value cfg_content = config.at("YOLOV6");
    ...
    if (!cfg_content.contains("model_keep_top_k")) {
        ...
        _m_keep_topk = static_cast<long>(cfg_content.at("model_keep_top_k").as_integer());
    }
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::init(const decltype(toml::parse(""))& cfg) {
