...
StatusCode YoloV6Detector<INPUT, OUTPUT>::Impl::init(const toml::table& config) {
    ...
    LOG(INFO) << "YoloV6 detection model initialization complete!!!";
...
yolov6_impl::internal_output YoloV6Detector<INPUT, OUTPUT>::Impl::decode_output_tensor() const {
    ...
    MNN::Tensor output_tensor_user(_m_net.output("outputs"), MNN::Tensor::DimensionType::CAFFE);
    _m_net.output("outputs")->copyToHostTensor(&output_tensor_user);
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::run_impl(const INPUT& input, OUTPUT& output) {
