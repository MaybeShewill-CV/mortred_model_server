...
class YoloV6Detector<INPUT, OUTPUT>::Impl {
public:
    /***
     *
     */
...
public:
     ...
     * @return
     */
    yolov6_impl::internal_output decode_output_tensor() const;
};
...
*/
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::Impl::init(const toml::table& config) {
    if (!config.contains("YOLOV6")) {
        LOG(ERROR) << "Config file does not contain YOLOV6 section";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }

    const toml::table* cfg_content_ptr = config["YOLOV6"].as_table();
    if (cfg_content_ptr == nullptr) {
        LOG(ERROR) << "Config section YOLOV6 missing or not a table";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }
    ...
    if (!FilePathUtil::is_file_exist(_m_model_file_path)) {
        LOG(ERROR) << "YoloV6 Detection model file: " << _m_model_file_path << " not exist";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }
    ...
    if (nullptr == _m_net) {
        LOG(ERROR) << "Create yolov6 detection model interpreter failed";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }
    ...
    _m_output_tensor = _m_net->getSessionOutput(_m_session, "outputs");

    if (_m_input_tensor == nullptr) {
        LOG(ERROR) << "Fetch yolov6 detection model input node failed";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }

    if (_m_output_tensor == nullptr) {
        LOG(ERROR) << "Fetch yolov6 detection model output node failed";
        _m_successfully_initialized = false;
        return StatusCode::MODEL_INIT_FAILED;
    }
    ...
    if (!cfg_content.contains("model_keep_top_k")) {
        ...
        _m_keep_topk = static_cast<long>(cfg_content["model_keep_top_k"].value_or<int64_t>(0));
    }
    ...
    LOG(INFO) << "YoloV6 detection model: " << FilePathUtil::get_file_name(_m_model_file_path)
 ...
 */
template<typename INPUT, typename OUTPUT>
cv::Mat YoloV6Detector<INPUT, OUTPUT>::Impl::preprocess_image(const cv::Mat& input_image) const {
    // resize image
    cv::Mat tmp;
...
*/
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::Impl::run(const INPUT& in, OUTPUT& out) {
    // transform external input into internal input
    auto internal_in = yolov6_impl::transform_input(in);
    ...
    out = yolov6_impl::transform_output<OUTPUT>(nms_result);
    return StatusCode::OK;
}
...
*/
template<typename INPUT, typename OUTPUT>
yolov6_impl::internal_output YoloV6Detector<INPUT, OUTPUT>::Impl::decode_output_tensor() const {
 ...
 */
template<typename INPUT, typename OUTPUT>
YoloV6Detector<INPUT, OUTPUT>::~YoloV6Detector() = default;
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::init(const toml::table& cfg) {
 ...
 */
template<typename INPUT, typename OUTPUT>
bool YoloV6Detector<INPUT, OUTPUT>::is_successfully_initialized() const {
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode YoloV6Detector<INPUT, OUTPUT>::run(const INPUT& input, OUTPUT& output) {
