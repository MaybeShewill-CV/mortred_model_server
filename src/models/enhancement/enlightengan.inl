...
StatusCode EnlightenGan<INPUT, OUTPUT>::Impl::init(const toml::table& config) {
    ...
    auto init_status = _m_net.init(cfg_content, {"input_src", "input_gray"}, {"output"});
...
StatusCode EnlightenGan<INPUT, OUTPUT>::Impl::run(const INPUT& in, OUTPUT& out) {
    ...
    if (!CvUtils::copy_image_to_tensor(input_tensor_data, input_src_chw_data, input_tensor_size)) {
        return StatusCode::MODEL_EMPTY_INPUT_IMAGE;
    }
    ...
    if (!CvUtils::copy_image_to_tensor(input_tensor_data, input_gray, input_tensor_size)) {
        return StatusCode::MODEL_EMPTY_INPUT_IMAGE;
    }
 ...
 */
template<typename INPUT, typename OUTPUT>
StatusCode EnlightenGan<INPUT, OUTPUT>::run_impl(const INPUT& input, OUTPUT& output) {
