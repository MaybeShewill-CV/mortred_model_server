...
std::vector<NamedTensor> AutoEncoderKL<INPUT, OUTPUT>::make_inputs(const INPUT& input) {
    ...
    if (input.decode_data.size() * sizeof(float) != named.tensor.byte_size()) {
        LOG(ERROR) << "vae decode data element count " << input.decode_data.size()
                   << " mismatches session input " << input_info.to_string();
        return {};
    }
...
StatusCode AutoEncoderKL<INPUT, OUTPUT>::postprocess(const std::vector<NamedTensor>& outputs,
    ...
    if (tensor.shape.size() != 4) {
        LOG(ERROR) << "unexpected vae decoder output shape: "
    ...
    auto hwc_data = jinq::common::CvUtils::convert_to_hwc_vec<uint8_t>(
        decode_out, static_cast<int>(channels), static_cast<int>(height),
    ...
    const cv::Mat decode_image(cv::Size(static_cast<int>(width), static_cast<int>(height)),
                               mat_dtype, hwc_data.data());
    if (channels == 3) {
...
AutoEncoderKL<INPUT, OUTPUT>::AutoEncoderKL()
    : jinq::models::BackendCvModel<INPUT, OUTPUT>("AUTOENCODER_KL") {}
