...
// dbnet text region detection server tool
...
using jinq::factory::ocr::create_dbtext_detection_server;
...
int main(int argc, char** argv) {
    ...
    const auto& server_cfg = config.at("DBNET_SERVER");
    auto port = server_cfg.at("port").as_integer();
    LOG(INFO) << "serve on port: " << port;

    auto server = create_dbtext_detection_server("dbnet_det_server");
    auto status = server->init(config);
    if (status != jinq::common::StatusCode::OK) {
        LOG(INFO) << "dbtext detection server init failed";
        return -1;
    }
