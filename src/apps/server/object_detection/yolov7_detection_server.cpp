...
using jinq::factory::object_detection::create_yolov7_det_server;
...
int main(int argc, char** argv) {
    ...
    const auto& server_cfg = config.at("YOLOV7_DETECTION_SERVER");
    auto port = server_cfg.at("port").as_integer();
    LOG(INFO) << "serve on port: " << port;

    auto server = create_yolov7_det_server("yolov7_det_server");
    server->init(config);
