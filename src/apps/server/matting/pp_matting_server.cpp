...
using jinq::factory::matting::create_pp_matting_server;
...
int main(int argc, char** argv) {
    ...
    const auto& server_cfg = config.at("PP_MATTING_SERVER");
    auto port = server_cfg.at("port").as_integer();
    LOG(INFO) << "serve on port: " << port;

    auto server = create_pp_matting_server("pp_matting_server");
    auto status = server->init(config);
    if (status != jinq::common::StatusCode::OK) {
        LOG(INFO) << "pp matting server init failed";
        return -1;
    }
