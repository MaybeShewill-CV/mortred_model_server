...
void vis_and_save(const mat_input& in, const std_feature_point_output& out, const std::string& image_path) {
    cv::Mat vis_image = in.input_image.clone();
    jinq::common::CvUtils::vis_feature_points(vis_image, out, 4);
...
int main(int argc, char** argv) {
    jinq::apps::BenchmarkSpec<mat_input, std_feature_point_output> spec;
    spec.model_name = "superpoint";
    spec.display_name = "superpoint extractor";
    ...
    spec.make_input = [](int argc, char** argv, const toml::table&) {
        return jinq::apps::make_single_image_input(argc, argv, "../demo_data/model_test_input/feature_point/test.png");
    };
    spec.image_path_of = [](int argc, char** argv) {
        return jinq::apps::standard_image_path(argc, argv, "../demo_data/model_test_input/feature_point/test.png");
    };
    spec.make_model = [](const std::string& model_name) {
        return jinq::factory::feature_point::create_superpoint_extractor<mat_input, std_feature_point_output>(model_name);
    };
