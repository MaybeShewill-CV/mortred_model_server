...
int main(int argc, char** argv) {
    ...
    if (argc == 3) {
        ...
        input_image_path = "../demo_data/model_test_input/object_detection/horses.jpg";
    ...
    // construct detector
    auto detector = create_yolov7_detector<mat_input, std_object_detection_output>("yolov7");
    auto cfg = toml::parse(cfg_file_path);
    detector->init(cfg);

    if (!detector->is_successfully_initialized()) {
        LOG(INFO) << "yolov7 detector init failed";
        return -1;
    }
    ...
    LOG(INFO) << "start yolov7 benchmark at: " << Timestamp::now().to_format_str();
