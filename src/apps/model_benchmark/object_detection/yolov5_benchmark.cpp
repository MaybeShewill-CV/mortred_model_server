...
void vis_and_save(const mat_input& in, const std_object_detection_output& out, const std::string& image_path) {
    ...
    output_file_name = output_file_name.substr(0, output_file_name.find_last_of('.')) + "_yolov5_result.png";
...
int main(int argc, char** argv) {
    ...
    spec.model_name = "yolov5";
    spec.display_name = "yolov5 detector";
    ...
    spec.make_model = [](const std::string& model_name) {
        return jinq::factory::object_detection::create_yolov5_detector<mat_input, std_object_detection_output>(model_name);
    };
