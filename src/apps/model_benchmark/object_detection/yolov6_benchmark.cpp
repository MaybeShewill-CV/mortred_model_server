...
int main(int argc, char** argv) {
    ...
    // construct detector
    auto detector = create_yolov6_detector<mat_input, std_object_detection_output>("yolov6");
    auto cfg = toml::parse(cfg_file_path);
    detector->init(cfg);

    if (!detector->is_successfully_initialized()) {
        LOG(INFO) << "yolov6 detector init failed";
        return -1;
    }
    ...
    LOG(INFO) << "start yolov6 benchmark at: " << Timestamp::now().to_format_str();
