...
int main(int argc, char** argv) {
    ...
    if (argc == 3) {
        ...
        input_image_path = "../demo_data/model_test_input/enhancement/real_esr/test.jpg";
    ...
    auto enhancementor = create_realesrgan_enhancementor<mat_input, std_enhancement_output>("real-esrgan");
    auto cfg = toml::parse(cfg_file_path);
    enhancementor->init(cfg);

    if (!enhancementor->is_successfully_initialized()) {
        LOG(INFO) << "real-esrgan enhancementor init failed";
        return -1;
    }
    ...
    LOG(INFO) << "start real-esrgan benchmark at: " << Timestamp::now().to_format_str();
    ...
    std::string output_path = FilePathUtil::concat_path("../demo_data/model_test_input/enhancement/real_esr/", output_file_name);
