...
* File: resnet_benchmark.cpp
* Date: 22-6-14
************************************************/

// resnet benckmark tool
...
int main(int argc, char** argv) {
    ...
    // construct detector
    auto classifier = create_resnet_classifier<mat_input, std_classification_output>("resnet");
    auto cfg = toml::parse(cfg_file_path);
    classifier->init(cfg);

    if (!classifier->is_successfully_initialized()) {
        LOG(INFO) << "resnet classifier init failed";
        return -1;
    }
    ...
    LOG(INFO) << "start resnet benchmark at: " << Timestamp::now().to_format_str();
