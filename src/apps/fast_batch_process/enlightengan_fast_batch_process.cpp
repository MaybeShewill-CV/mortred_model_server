...
static std::unique_ptr<BaseAiModel<mat_input, std_enhancement_output> > get_enhancementor() {}
    ...
    static auto enhancementor = create_enlightengan_enhancementor<mat_input, std_enhancement_output>("enlightengan");
...
void enhance_frame(
        cv::Mat& input_image,
        const std::string& vis_save_path,
        ProcessImgTaskOutput& process_task_output) {
    
    mat_input model_input {
        input_image
    };
 ...
 * @param ret
 */
void save_frame(const ProcessImgRet& ret) {
    cv::imwrite(ret.save_path, ret.output_image);
}
 ...
 */
void process_task(
        ProcessImgTaskInput& process_task_input,
        ProcessImgTaskOutput& process_task_output,
        const std::string& vis_save_dir,
...
void parallel_enhance_lowlight(const std::string& input_dir, const std::string& save_dir) {
    ...
    for (int index = 0; index < producer_count; ++index) {
        auto* producer = WFTaskFactory::create_go_task(
                "producer", read_task,
                std::ref(read_task_input),
                std::ref(read_task_output));
    ...
    auto processimg_task = WFTaskFactory::create_go_task(
            "process img worker", process_task,
            std::ref(read_task_output),
            std::ref(process_task_output),
            std::ref(save_dir),
            std::ref(no_more_data_will_be_readed));
    ...
    auto* p_saveimg_work = Workflow::create_parallel_work([&](const ParallelWork * work) {
        LOG(INFO) << "p_save_img_work complete and all save img workers exit";
    });
    const int save_img_worker_count = 2;
    for (int index = 0; index < save_img_worker_count; ++index) {
        auto* saver = WFTaskFactory::create_go_task(
                "save img worker", save_task,
                std::ref(process_task_output),
    ...
    wait_group.wait();
    LOG(INFO) << "all parallel cost time: "
              << TIME_COST(t_start, std::chrono::system_clock::now()) / 1000.0 << " seconds";
